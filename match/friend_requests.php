...
if ($numrows == 0) {
 echo "<h1 class='text-center'>You have no matches yet.</h1>";
 $user_from = "";
}
else
{
 while ($get_row = mysql_fetch_assoc($friendRequests)) {
  ...
  echo '' . $user_from . ' wants to be add you'.'<br />';
...
if (isset($_POST['acceptrequest'.$user_from])) {
  ...
  echo "<h1 class='text-center'>You are now nomoked!</h1>";
...
$ignore_request = mysql_query("DELETE FROM friend_requests WHERE user_to='$user_to'&&user_from='$user_from'");
  echo "<h1 class='text-center'>Request Ignored!</h1>";
  header("Location: friend_requests.php");
}
?>
