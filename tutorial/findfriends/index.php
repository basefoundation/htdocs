...
if($reg) {
	if($em==$em2) {
		//Check the amount of rows where user=$un
		$u_check = mysql_query("SELECT username FROM users WHERE username='$un'");
		//Count the amount of rows where user=$un
		...
		if($check==0) {
			...
			if($fn&&$ln&&$un&&$em&&$em2&&$pswd&&$pswd2) {
				...
				if($pswd==$pswd2) {
					//check the maximum length of user/first name/last name does not exceed 25 characters
				if(strlen($un)>25||strlen($fn)>25||strlen($ln)>25) {
					echo "The maximum limit for user/first name/last name is 25 characters!";
				}	
				else {
